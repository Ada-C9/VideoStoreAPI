...
describe RentalsController do
  describe "check_out" do
    ...
    it "returns an error if movie is already checked out by that customer" do
      assert_no_difference "Rental.count" do
        post check_out_path, params: { movie_id: movies(:HP).id, customer_id: customers(:kari).id }
    ...
    it "permits a customer to checkout a previously rented movie if it has been returned" do
      assert_difference "Rental.count", 1 do
        post check_out_path, params: { movie_id: movies(:HP).id, customer_id: customers(:dan).id }
        must_respond_with :success
      end
  ...
  describe "check_in" do
    ...
    it "returns an error if previously rented movie is already returned" do
      assert_no_difference "Rental.count" do
        post check_in_path,
