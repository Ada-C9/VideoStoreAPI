...
describe Movie do
  ...
  describe "validations" do
    ...
    it "must have both title and release_date and inventory" do
  ...
  describe 'a_checkout' do
    it 'must reduce inventory by single value if current available_inventory > 0' do
      phantom_thread = movies(:Phantom)
      assert_operator phantom_thread.available_inventory, :>, 0
      proc{
        phantom_thread.a_checkout
      }.must_change 'phantom_thread.available_inventory', -1

    end

    it 'will return false on attempt to reduce stock on movies w/available_inventory of 0' do
      grease = movies(:Grease)
      grease.available_inventory.must_equal 0
      proc{
        grease.a_checkout
